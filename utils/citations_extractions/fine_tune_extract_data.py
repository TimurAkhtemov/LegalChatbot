...
def generate_response(prompt):
    ...
    if "usage" in response_json:
        ...
        if "time_to_first_token" in usage:
            print(f"Time to first token: {usage['time_to_first_token']:.2f}s")
        if "tokens_per_second" in usage:
...
### **Example 8**
Text:
  The provisions of the Revised Statutes, not inconsistent with those of prior  laws, and the provisions of any revision law not inconsistent with those of any  of the laws revised therein, shall be construed as a continuation of such prior  laws.       Amended by L.1960, c. 187, p. 780, s. 4. 
...
### **Example 9**
Text:
  This section tests multiple amended legislative actions. amended 1988, c.44, s.7; 2005, c.343; 2008, c.84, s.2; repealed 2019, c.276, s.20; amended 2021, c.16, s.62.
...
### **Example 10**
Text:
  an electric plant or works has at any prior time be  submitted to and approved by the legal voters of such municipality in  accordance with requirements of R.S. 40:62-15.       L.1973, c. 280, s. 4, eff. Nov. 29, 1973.
...
### **Example 11**
Text: 
   submitted to and approved by the legal voters of such municipality in with a new section being included in the subsequent election period.
...
def remove_citation(truncated_text, original_text, extracted_citation, buffer=8, augmenting=False):
    ...
    if stripped_citation == "None":
        sample = {
            "prompt": f"Extract citations from: {truncated_text}",
            "completion": extracted_citation
        }
        if not augmenting:
            fine_tuning_data.append(sample)
...
def process_statute_citations(file):
    ...
    for title in file:
        for section in title['sections']:
            ...
            try:
                ...
                text_removed_citations = remove_citation(truncated_text, text, citations) if citations != "None" else text
    ...
    with open('data/processed/fine_tuning_data.json', 'w') as f:
